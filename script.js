...
function updateFilterOptions() {
    // 名前フィルターの更新
    const names = [...new Set(allShiftData.map(item => item.name))].sort();
    const nameFilter = document.getElementById('nameFilter');
    if (nameFilter) {
...
function displayShiftData() {
    ...
    // 表示モードに応じてデータを表示
    if (viewMode && viewMode.value === 'weekly') {
        displayWeeklyShiftData();
    } else {
...
// 週別表示の関数
function displayWeeklyShiftData() {
    const shiftData = document.getElementById('shiftData');
    const pagination = document.getElementById('pagination');
    ...
    filteredData.forEach(item => {
        if (!employeeGroups[item.name]) {
            employeeGroups[item.name] = [];
        }
        employeeGroups[item.name].push(item);
    });

    // 週別表示のHTMLを作成
    let weeklyHTML = '<div class="weekly-shift-container">';
    
    // 週の範囲を表示するヘッダーを追加
    const weekRange = formatWeekRange(window.currentWeekStart || new Date());
    weeklyHTML += `
        <div class="weekly-header">
            <h3><i class="fas fa-calendar-week"></i> ${weekRange}</h3>
        </div>
    `;
    
    Object.keys(employeeGroups).forEach(employeeName => {
        const shifts = employeeGroups[employeeName];
        
        // 日付順にソート
        shifts.sort((a, b) => new Date(a.date) - new Date(b.date));
        
        weeklyHTML += `
            <div class="weekly-employee-card">
                <div class="employee-header">
                    <h4>${employeeName}</h4>
                    <span class="shift-count">${shifts.length}日間</span>
                </div>
                <div class="weekly-shift-grid">
        `;
        
        // 一週間分のシフトを表示（月曜日から日曜日）
        const weekDays = ['月', '火', '水', '木', '金', '土', '日'];
        
        // 現在の週の開始日を取得（デフォルトは今週）
        let startOfWeek;
        if (window.currentWeekStart) {
            ...
            startOfWeek.setDate(today.getDate() - today.getDay() + 1); // 月曜日を取得
        ...
        for (let i = 0; i < 7; i++) {
            ...
            // その日のシフトを検索
            const dayShift = shifts.find(shift => shift.date === dateString);
            
            if (dayShift) {
                const checkInColor = dayShift.check_in_color || '#FFFFFF';
                const checkOutColor = dayShift.check_out_color || '#FFFFFF';
                const checkInStoreName = getStoreNameFromColor(checkInColor);
                const checkOutStoreName = getStoreNameFromColor(checkOutColor);
                        ...
                        <div class="shift-day work-day">
                            ...
                            <div class="shift-details">
                                <div class="shift-time">
                                    ...
                                    <span class="time-value">${dayShift.check_in_time || '-'}</span>
                                </div>
                                <div class="shift-time">
                                    <span class="time-label">退勤:</span>
                                    <span class="time-value">${dayShift.check_out_time || '-'}</span>
                                </div>
                                <div class="store-info">
                                    <span class="color-indicator" style="background-color: ${checkInColor};"></span>
                                    <span class="store-name">${checkInStoreName}</span>
                                </div>
                            </div>
                        </div>
                    `;
    ...
    weeklyHTML += '</div>';
    shiftData.innerHTML = weeklyHTML;
...
function changeWeek(weekOffset) {
    ...
    const newWeekStart = new Date(window.currentWeekStart);
    newWeekStart.setDate(newWeekStart.getDate() + (weekOffset * 7));
    window.currentWeekStart = newWeekStart;
    
    displayShiftData();
    showNotification(`${formatWeekRange(window.currentWeekStart)}のシフトを表示中`, 'info');
}
...
function resetToCurrentWeek() {
    ...
    displayShiftData();
    showNotification('今週のシフトを表示中', 'info');
}
